...
pub fn to_ir(
    ...
    match expr.expr {
        ...
        Expr::Access(left, right) => {
            access(left, right, ir, block, module, store, locals, next_var)
        }
        Expr::Let(name, value_expr) => {
            variable_declaration(name, value_expr, ir, block, module, store, locals, next_var)
        }
        Expr::Block(statements, returns) => compile_block(
            statements, returns, ir, block, module, store, locals, next_var, tail,
        ),
        Expr::InitializeStruct(name, fields) => {
            initialize_struct(name, fields, ir, block, module, store, locals, next_var)
        }
        Expr::Return(value_expr) => returns(value_expr, ir, block, module, locals, next_var),
        Expr::Call(callee, args) => call(
            callee, args, ir, block, module, store, locals, next_var, tail,
        ),
        Expr::Name(name) => variable(name, ir, block, store, locals),
        Expr::If { cond, then, els } => conditional(
            cond, then, els, expr.idx, ir, block, module, store, locals, next_var, tail,
        ),
        Expr::Guard { dependency, body } => guard(
            dependency, body, ir, block, module, store, locals, next_var, tail,
        ),
        Expr::Index(left, right) => index(left, right, ir, block, module, store, locals, next_var),
